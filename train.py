...
parser.add_argument('--val_interval', default=10000, type=int, help='Evaluate and save the model every [val_interval] iterations, pass -1 to disable.')
...
def save_best(net):
    weight = glob.glob('/content/drive/MyDrive/交大電子109上/深度學習_影像/Homework3/weights/best*')
...
def save_latest(net):
    weight = glob.glob('/content/drive/MyDrive/交大電子109上/深度學習_影像/Homework3/weights/latest*')
    torch.save(net.state_dict(), '/content/drive/MyDrive/交大電子109上/深度學習_影像/Homework3/weights/latest_'+cfg.name+'_'+str(step)+'.pth')
...
if args.resume == 'latest':
    weight = glob.glob('/content/drive/MyDrive/交大電子109上/深度學習_影像/Homework3/weights/latest*')[0]
    ...
    resume_step = int('weights/' +args.resume.split('.pth')[0].split('_')[-1])
...
data_loader = data.DataLoader(dataset, cfg.batch_size, shuffle=True, num_workers=8, collate_fn=detection_collate, pin_memory=True)
...
try:  # Use try-except to use ctrl+c to stop and save early.
    while training:
        for i, datum in enumerate(data_loader):
            ...
            if args.val_interval > 0 and step % val_interval == 0 and step != start_step:
