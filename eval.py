...
parser.add_argument('--trained_model', default='latest_res101_custom_110000.pth', type=str)
...
class Make_json:
    def __init__(self):
        ...
        for coco_id, real_id in PASCAL_LABEL_MAP.items():
            class_id = real_id - 1
    ...
    def dump(self):
        dump_arguments = [(self.bbox_data, '/content/drive/MyDrive/DNN_HW3/results/bbox_detections.json'),
                          (self.mask_data, '/content/drive/MyDrive/DNN_HW3/results/mask_detections.json')]
...
def evaluate(net, img_path, info,  max_num=-1, during_training=False, cocoapi=False, traditional_nms=False):
    ...
    for imgid in cocoGt.imgs:
        ...
        with timer.env('Data loading'):
            img = cv2.imread(img_path + cocoGt.loadImgs(ids=imgid)[0]['file_name']) # load_image
            h, w, depth = img.shape
            # Basequgmentation
            img = img.astype(np.float32) 
            expand_image = np.zeros((cfg.img_size, cfg.img_size, depth), dtype=img.dtype) # Pad
            expand_image[:, :, :] = MEANS
            expand_image[:h, :w] = img
            img = expand_image
            img = cv2.resize(img, (cfg.img_size, cfg.img_size)) # resize
            # BackboneTransform
            mean_trans = np.array(MEANS, dtype=np.float32)
            std_trans = np.array(STD, dtype=np.float32)
            img = (img - mean_trans) / std_trans
        ...
        with timer.env('Network forward'):
            ...
            with timer.env('Output json'):
              ...
              for i in range(pred_masks.shape[0]):
                # Make sure that the bounding box actually makes sense and a mask was produced
                if (pred_boxes[i, 3] - pred_boxes[i, 1]) * (pred_boxes[i, 2] - pred_boxes[i, 0]) > 0:
                    make_json.add_bbox(imgid, pred_classes[i], pred_boxes[i, :], pred_confs[i])
                    make_json.add_mask(imgid, pred_classes[i], pred_masks[i, :, :], pred_confs[i])
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        net = Yolact()
        net.load_weights('/content/drive/MyDrive/DNN_HW3/' + args.trained_model, cuda)
        net.eval()
        print('\nModel loaded.\n')
        ...
        evaluate(net, '/content/drive/MyDrive/DNN_HW3/test_images/', cfg.dataset.valid_info, args.max_num, False, args.cocoapi, args.traditional_nms)